"""A Simple Snake Game.2018.10.22."""
...
__version__ = "1.8.6 BETA"


class SnakeGame(object):
    """A snake game can be played while waiting the queueself."""
    def __init__(self, master):
        """
        Initial a snake game master.
        ...
        self._goal = tk.Label(top_frame, text='SCORE: 0',
                              bg='black', fg='white')
        self._goal.pack(side=tk.LEFT)
        self._hiscore_lbl = tk.Label(top_frame, text='HI-SCORE: 0',
                                     bg='black', fg='white')
        ...
        self._title = tk.Label(self._intro, text='Welcome', font=20,
                               bg='lightblue')
        self._title.pack(ipadx=50, ipady=10)
        self._text = tk.Label(self._intro, text="Use 'w','a','s','d' to"
                              " control the snake.\nPress 'q' to pauser"
                              "\nPress '1' to start\nPress '2' to "
                              "show leaderboard",
        ...
        try:
            ...
            messagebox.showinfo('Warning', 'Cannot connect to the server,'
                                ' you will not be able to access Laderboard '
                                'or add your record into leaders')
        self._leader_frame = tk.Frame(self._cv)
        self._leader_frame.config(bg='lightblue')
        leader_title = tk.Label(self._leader_frame, text="LeaderBoard",
                                font=20, bg='lightblue')
        leader_title.grid(row=0, column=0, columnspan=3)
        self._leader_lines = []
        for rank in range(1, 11):
            rank_lbl = tk.Label(self._leader_frame, text='-',
                                bg='lightblue', padx=2, pady=1)
            rank_lbl.grid(row=rank, column=0)
            name_lbl = tk.Label(self._leader_frame, text='---',
                                bg='lightblue', padx=2, pady=1)
            name_lbl.grid(row=rank, column=1)
            score_lbl = tk.Label(self._leader_frame, text='---',
                                 bg='lightblue', padx=2, pady=1)
            score_lbl.grid(row=rank, column=2)
            self._leader_lines.append((rank_lbl, name_lbl, score_lbl))
        leader_exit = tk.Label(self._leader_frame, text="Press '2' again to "
                               "close leaderboard", bg='lightblue')
        leader_exit.grid(row=11, column=0, columnspan=3, ipady=3)
        ...
        # self._alive = 0 means snake is dead,
        # self._alive = 1 means snake is alive, self._alive = -1 means paused
        self._alive = 0
        self._eating = False
        self._auto = False

        self._food = None
        self._food_x, self._food_y = -1, -1

    def start_game(self):
        """
        The reset function to reset all variable to original statue
        and start game."""
        # reset all variables
        ...
        for i in range(0, 6):
            ...
            self._y += self._dy
            dot = self._cv.create_rectangle(self._x, self._y, self._x + 10,
                                            self._y + 10, fill='white')
    ...
    def get_input(self, e):
        """Get different input to control the game, snake cannot move backward
            ...
            press s to go down
            press d to go left
            press 1 while game over to restart
            press 2 to visit leaderboard or close leader
            press 3 to add current record into leader
        """
        if (e.char == 'w' and self._dy != 10):
            self._dx, self._dy = 0, -10
        elif (e.char == 'a' and self._dx != 10):
            self._dx, self._dy = -10, 0
        elif (e.char == 's' and self._dy != -10):
            self._dx, self._dy = 0, 10
        elif (e.char == 'd' and self._dx != -10):
            self._dx, self._dy = 10, 0
        elif e.char == 'q':
            self.pause()
        elif e.char == 'f':
            self._auto = True
        elif (e.char == '1' and self._alive == 0):
            self.start_game()
            self.main_control()
        elif (e.char == '2' and self._alive == 0 and self._leader_statue):
            self.show_leader()
        elif (e.char == '3' and self._alive == 0 and self._leader_statue):
            self.set_new_leader()
        else:
            pass

    def create_food(self):
        """reate a food to a random position."""

        food_x = 10 * random.randint(0, 24)
        food_y = 10 * random.randint(0, 29)
        if len(self._cv.find_overlapping(food_x + 3, food_y + 3, food_x + 6,
                                         food_y + 6)) > 0:
            self.create_food()
        else:
            self._food = self._cv.create_rectangle(food_x, food_y, food_x + 10,
                                                   food_y + 10, fill='red')
            self._food_x = food_x
            self._food_y = food_y
    ...
    def auto_move(self):
        """Should be the auto move function."""
        pass
    ...
    def main_control(self):
        ...
        # when head of snake hit something.
        # if something is food, eat and become longer, else, game over.
        if len(self._cv.find_overlapping(self._x + 3, self._y + 3, self._x + 6,
                                         self._y + 6)) > 1:
            for dot in self._cv.find_overlapping(self._x + 3, self._y + 3,
                                                 self._x + 6, self._y + 6):
                if dot == self._food:
                    self._eating = True
        ...
        if self._alive == 1:
            ...
            self._text.config(text="Press '1' to restart\n"
...
class LeaderData(object):
    ...
    def __init__(self):
        ...
        self._con = mysql.connector.connect(host="localhost",
                                            user="root",
                                            passwd="Didi1234",
                                            database="snakey")
    ...
    def get_leader(self):
        ...
        self._leaderboard.execute("SELECT id,name,hi_score FROM temp_leader "
